...
from noteforge.subtitle.parser import (
    SrtSubtitleParser,
    VttSubtitleParser,
    parse_subtitle,
)
...
def _subtitle_file(
    ...
    *,
    ...
    path = tmp_path / f"subtitle.{extension}"
...
def test_parses_srt_sequence_timestamps_multiline_and_tags(
    tmp_path: Path,
) -> None:
    subtitle = _subtitle_file(
        tmp_path,
...
""",
    ...
    transcript = SrtSubtitleParser().parse(subtitle)

    assert transcript.language == "zh-CN"
    assert transcript.source == "automatic_subtitle"
    assert len(transcript.segments) == 2
    assert transcript.segments[0].start_seconds == 1.25
    assert transcript.segments[0].end_seconds == 3.5
    assert transcript.segments[0].text == "大家好\n欢迎来到今天的课程"
...
def test_invalid_srt_timestamp_raises_parse_error(
    tmp_path: Path,
    timing: str,
) -> None:
    subtitle = _subtitle_file(
    ...
    with pytest.raises(SubtitleParseError):
        SrtSubtitleParser().parse(subtitle)


def test_parse_subtitle_dispatches_srt_parser(tmp_path: Path) -> None:
    subtitle = _subtitle_file(
        tmp_path,
    ...
    assert transcript.segments[0].text == "正文"

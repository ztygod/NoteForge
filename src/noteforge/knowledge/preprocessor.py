...
class PreprocessedChunk:
    ...
    def __post_init__(self) -> None:
        ...
        if not all(isinstance(chunk, RawChunk) for chunk in self.source_chunks):
            raise TypeError("source_chunks 中的元素必须全部是 RawChunk")
        if self.start_time != min(chunk.start_time for chunk in self.source_chunks):
            raise ValueError("开始时间必须与来源文本块一致")
        if self.end_time != max(chunk.end_time for chunk in self.source_chunks):
...
class ChunkPreprocessor:
    ...
    def __init__(
        ...
        if not isfinite(gap_threshold_seconds) or gap_threshold_seconds < 0:
    ...
    def preprocess(
        ...
        for chunk in source_chunks[1:]:
            ...
            if gap < self._gap_threshold_seconds and self._can_merge(
                current_group,
                current_parts,
                current_end,
                chunk,
    ...
    def _validate_time_order(chunks: tuple[RawChunk, ...]) -> None:
        ...
        for previous, current in zip(chunks, chunks[1:]):
            if current.start_time < previous.start_time:
                raise ValueError("RawChunk 必须按 start_time 非递减顺序输入")
    ...
    def _can_merge(
        ...
        merged_characters = current_characters + separator_length + len(next_text)

...
from noteforge.media.models import Subtitle, VideoMetadata, VideoResource
...
class PlatformCollector(ABC):
    ...
    def extract(
        ...
        *,
        ...
        if not transcript:
            ...
            if selected:
                selected = self._download_subtitle(
                    metadata.webpage_url, metadata, selected
                )
        ...
        if not transcript and self.transcriber:
            ...
            transcript = self.transcriber.transcribe(
                audio_path, language=subtitle_language
            )
            self.cache.save_transcript(metadata, transcript)
            transcript_source = "whisper"
        video_path = self._download_media(metadata, False) if download_video else None
        return VideoResource(
            metadata, subtitles, transcript, audio_path, video_path, transcript_source
        )

    def select_subtitle(
        self, subtitles: tuple[Subtitle, ...], preferred: str | None
    ) -> Subtitle | None:
    ...
    def _download_subtitle(
        self, source: str, metadata: VideoMetadata, subtitle: Subtitle
    ) -> Subtitle:
        ...
        if subtitle.content is not None:
            ...
            return Subtitle(
                ...
                path,
                subtitle.content,
                subtitle.is_automatic,
            )
        info = self.client.download_subtitle(
            source,
        ...
        item = (
            requested.get(subtitle.language, {})
        ...
        return Subtitle(
            subtitle.language,
            str(item.get("ext", subtitle.format)),
            Path(path),
            is_automatic=subtitle.is_automatic,
        )
    ...
    def _download_media(self, metadata: VideoMetadata, audio: bool) -> Path:
        ...
        info = self.client.download_media(
            metadata.webpage_url, target_dir=target, audio_only=audio
        )
        downloads = info.get("requested_downloads")
        path = (
            Path(str(downloads[0].get("filepath", "")))
            if isinstance(downloads, list) and downloads
            else Path(str(info.get("_filename", "")))
        )

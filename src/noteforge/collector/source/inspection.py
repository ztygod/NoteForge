...
def inspect_source(source: str) -> InspectionResult:
    ...
    if parsed.scheme in {"http", "https"} and parsed.hostname in _BILIBILI_HOSTS:
        ...
        if (
            len(parts) >= 2
            and parts[0] == "video"
            and _BVID_PATTERN.fullmatch(parts[1])
        ):
            page = 1
            try:
            ...
            return InspectionResult(
                original, InspectionPlatform.BILIBILI, parts[1], normalized, page
            )
    ...
    if parsed.scheme in {"http", "https"}:
        ...
        if parsed.hostname in _YOUTUBE_SHORT_HOSTS and parts:
            ...
            parsed.hostname in _YOUTUBE_EMBED_HOSTS
            and len(parts) >= 2
            and parts[0] == "embed"
        ):
            video_id = parts[1]
